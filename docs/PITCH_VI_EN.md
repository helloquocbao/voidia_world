...
## Opening / Mo dau
- EN: We are HiddenKey. Voidia World is an on-chain world builder on Sui: every land tile (PLOTNFT) is a real NFT you can build, trade, and earn from.
- VI: Chúng tôi là HiddenKey. Voidia World là game xây thế giới on-chain trên Sui: mỗi ô đất (PLOTNFT) là NFT thật, có thể xây, giao dịch và kiếm PLOT.
...
- EN: We put land, edits, trading, and rewards fully on-chain. Loop: claim PLOT, build/edit, play to earn PLOT, trade, withdraw.
- VI: Đưa đất, chỉnh sửa, giao dịch, thưởng lên on-chain. Vòng lặp: nhận PLOT, xây/sửa, chơi nhận PLOT, mua/bán, rút tiền.
...
- EN: You mint land (PLOTNFT), edit it, and trade it for PLOT; the chain records ownership and payments.
- VI: Mint đất (PLOTNFT), sửa, mua/bán bằng PLOT; chuỗi ghi nhận chủ sở hữu và thanh toán.
...
- EN: Use world base difficulty (1-9) + PLOT count to set target enemy count. Every few seconds read Sui TPS via `getTotalTransactionBlocks` to know network load.
- VI: Dùng độ khó world (1-9) + số PLOT để đặt số quái mục tiêu. Mỗi vài giây đọc TPS Sui qua `getTotalTransactionBlocks` để biết mạng bận hay nhàn.
...
- EN: Pages: Landing, Game, Editor, Marketplace. WalletHeader shows PLOT + connect.
- VI: Trang: Landing, Game, Editor, Marketplace. WalletHeader hiển PLOT + nút connect.
- EN: Marketplace doc event + dynamic fields; show PLOT image; buy/list/delist; withdraw proceeds.
- VI: Marketplace đọc event + dynamic field; hiển ảnh PLOT; mua/list/hủy; rút proceeds.
...
1) EN: Edit a PLOT (`update_PLOT`: tiles + image).  
   VI: Sửa PLOT (`update_PLOT`: tiles + ảnh).
2) EN: List the PLOT with PLOT price.  
   VI: List PLOT, đặt giá PLOT.
...
5) EN (optional): `play_v2` + `claim_reward` to earn PLOT and boost stats.  
   VI (tùy chọn): `play_v2` + `claim_reward` để nhận PLOT và tăng chỉ số.
...
## Closing / Ket
- EN: HiddenKey builds Voidia World for real on-chain ownership and gameplay. Thank you.
- VI: HiddenKey xây Voidia World cho quyền sở hữu on-chain và gameplay thật. Cảm ơn.
